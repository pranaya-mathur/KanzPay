/**
 * CBD parser — same structure as other UAE banks; anti-bot handled at crawl layer.
 */
...
const PARSER = 'cbdParser';
const bankMeta = BANK_META.cbd;

export function matchCbd(url) {
    return /\bcbd\.ae\b/i.test(url);
}

export function parseCbd($, url, rawText, rawHtml, meta = {}) {
    const reason = `domain=cbd.ae; path=${safePathname(url)}`;
    const offers = [];
